...
static void draw_sprite_circles(game_t *game, sfRenderWindow *window, int *i)
{
    sfRenderWindow_drawCircleShape(window, game->villager->v_cir[*i], NULL);
    sfSprite_setTextureRect
    (game->villager->v_sp[*i], (sfIntRect) {0, 0, 217, 219});
    sfRenderWindow_drawSprite(window, game->villager->v_sp[*i], NULL);
}
...
void draw_villager(game_t *game, sfRenderWindow *window)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        draw_sprite_circles(game, window, &i);
    }
}

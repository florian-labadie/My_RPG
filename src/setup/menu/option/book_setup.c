...
int setup_open_book(settings_t *settings)
{
    ...
    if (!settings->anim->open_book)
        return KO;
    settings->anim->open_book_spr = create_button(settings->anim->open_book, (sfVector2f) {7.5, 3.75}, (sfVector2f) {25.0, -120.0});
    sfSprite_setTextureRect(settings->anim->open_book_spr, settings->react->rect);
...
int setup_next_page(settings_t *settings)
{
    ...
    if (!settings->anim->next_page)
        return KO;
    settings->anim->next_page_spr = create_button(settings->anim->next_page, (sfVector2f) {7.5, 3.75}, (sfVector2f) {25.0, -120.0});
    sfSprite_setTextureRect(settings->anim->next_page_spr, settings->react->ract);
    settings->anim->book_clock = sfClock_create();
    return OK;
}
...
int setup_previous_page(settings_t *settings)
{
    ...
    settings->anim->previous_page = sfTexture_createFromFile(PREVIOUS_PAGE_P, NULL);
    if (!settings->anim->previous_page)
        return KO;
    settings->anim->previous_page_spr = create_button(settings->anim->previous_page, (sfVector2f) {7.5, 3.75}, (sfVector2f) {25.0, -120.0});
    sfSprite_setTextureRect(settings->anim->previous_page_spr, settings->react->ruct);
    settings->anim->book_clock = sfClock_create();
    return OK;
}
...
int setup_close_book(settings_t *settings)
{
    ...
    if (!settings->anim->close_book)
        return KO;
    settings->anim->close_book_spr = create_button(settings->anim->close_book, (sfVector2f) {7.5, 3.75}, (sfVector2f) {25.0, -120.0});
    sfSprite_setTextureRect(settings->anim->close_book_spr, settings->react->rict);
    settings->anim->book_clock = sfClock_create();
    return OK;
}

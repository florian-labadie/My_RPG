...
static void setup_item_weapon_part2(item_t **item, sfRenderWindow *window,
    int *j)
{
...
static int setup_item_weapons(item_t **item, sfRenderWindow *window)
{
    ...
    for (int j = 0; j < 3; j += 1) {
        (*item)->weapons[0][j] = create_button((*item)->texture[1],
            get_resize(window, 3, 3),
            get_resize(window, 420 + (j * 125), 1060));
        sfSprite_setTextureRect((*item)->weapons[0][j], rects_gants[j]);
        set_sprite_mid_origin((*item)->weapons[0][j]);
        sfSprite_setColor((*item)->weapons[0][j],
            sfColor_fromRGB(100, 100, 100));
        setup_item_weapon_part2(item, window, &j);
    }
    return OK;
}

static void setup_item_potion_mana(item_t **item, sfRenderWindow *window)
{
    for (int i = 0; i < 4; i++) {
        (*item)->mana_pot[i] = create_button((*item)->texture[0],
            get_resize(window, 0.3, 0.3),
            get_resize(window, 260 + (i * 130), 580));
        sfSprite_setTextureRect((*item)->mana_pot[i], SLOTH_MANA);
        set_sprite_mid_origin((*item)->mana_pot[i]);
        sfSprite_setColor((*item)->mana_pot[i],
            sfColor_fromRGB(100, 100, 100));
    }
}
...
static int setup_item_potion(item_t **item, sfRenderWindow *window)
{
    ...
    for (int i = 0; i < 4; i++) {
        (*item)->health_pot[i] = create_button((*item)->texture[0],
            get_resize(window, 0.4, 0.4),
            get_resize(window, 265 + (i * 130), 320));
        sfSprite_setTextureRect((*item)->health_pot[i], SLOTH_HEALTH);
        set_sprite_mid_origin((*item)->health_pot[i]);
        sfSprite_setColor((*item)->health_pot[i],
            sfColor_fromRGB(100, 100, 100));
    }
    setup_item_potion_mana(item, window);
    return OK;
}

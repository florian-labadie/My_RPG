...
static void create_help_text(sfRenderWindow *window, menu_t *menu)
{
    sfVector2f text_pos;
    ...
    "Bienvenu dans le menu d'Aide", get_less_size(window, 35), text_pos);
}
...
void setup_help_menu(sfRenderWindow *window, menu_t *menu)
{
    ...
    sfSprite_setPosition(backboard_sprite, get_resize(window, 300, 350));
    sfSprite_setScale(backboard_sprite, get_resize(window, 3.0f, 2.5f));
